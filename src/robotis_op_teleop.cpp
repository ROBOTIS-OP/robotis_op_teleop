...
public:
    TBK_Node()
    {
        ...
        joint_states_sub_ = n_.subscribe("/robotis_op/joint_states", 100, &TBK_Node::jointStatesCb, this);


        pub_ = n_.advertise<geometry_msgs::Twist>("/robotis_op/cmd_vel",1);
        tilt_pub_ = n_.advertise<std_msgs::Float64>("/robotis_op/j_tilt_position_controller/command", 100);
        pan_pub_ = n_.advertise<std_msgs::Float64>("/robotis_op/j_pan_position_controller/command", 100);
        action_pub_ = n_.advertise<std_msgs::Int32>("/robotis_op/start_action", 100);
        enable_walking_pub_ = n_.advertise<std_msgs::Bool>("/robotis_op/enable_walking", 100);
    }
    ~TBK_Node() { }
    void keyboardLoop();
    void stopRobot()
